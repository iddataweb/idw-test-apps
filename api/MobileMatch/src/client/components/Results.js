...
const Results = ({ results }) => {
  ...
  return (
    ...
    <pre>Transaction ID: <a target="_blank" href={`https://preprod.admin.iddataweb.com/axnadmin/app/reports/verification-transaction/details/${results.transaction_id}`}>{results.transaction_id}</a></pre>
    <pre>...</pre>
    <button onClick={toggleAllResults}>Expand Results</button>
    <button onClick={() => window.open(`https://preprod.admin.iddataweb.com/axnadmin/app/reports/verification-transaction/details/${results.transaction_id}`, "_blank")}>View Full Transaction</button>
